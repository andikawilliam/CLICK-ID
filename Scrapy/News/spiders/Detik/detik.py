...
class DetikSpider(scrapy.Spider): 
	name = "Detik"	# name of spider

	# Dates of the news pages to scrap
	start_urls = [
	'https://news.detik.com/indeks/all/1?date=09/%s/2019' % i for i in range(10,30)
	# 'https://finance.detik.com/indeks?date=09%2F01%2F2019' % i for i in range(10,11)
	...
	def parse_details(self, response):
		...
		# Detik.com has a peculiar feature
		# It has two date and time formats 
		# one is '2019/09/01 23:49:43 WIB' and the other 'Minggu 08 September 2019, 20:27 WIB'
		# Therefore we need to add an if else clause, in the 1st format the 5th char is always '/'
		if(src_date[4] == '/'):
			date = src_date[:10]
			time =  src_date[11:-7]
		else:
			date = src_date.rsplit(', ', 1)[0] # split into 'Minggu 08 September 2019'
			date = date.split(' ', 1)[1] 	   # retrieves only '08 Septermber 2019'

			time = src_date.rsplit(', ', 1)[1] # split into '20:27 WIB'
			time = time.split(' ', 1)[0] 	   # retrieves only '20:27'
